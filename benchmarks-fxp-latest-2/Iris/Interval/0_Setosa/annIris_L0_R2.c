...
int main() {
  ...
  int x0 = nondet_int();
  __ESBMC_assume((x0 >= 481) && (x0 <= 499));
  int x1 = nondet_int();
  __ESBMC_assume((x1 >= 294) && (x1 <= 306));
  int x2 = nondet_int();
  __ESBMC_assume((x2 >= 138) && (x2 <= 142));
  int x3 = nondet_int();
  __ESBMC_assume((x3 >= 20) && (x3 <= 20));
  ...
  layer1[0] = r0;
  __ESBMC_assume((layer1[0] >= fxp_float_to_fxp(-0.846394002438)) &&
                 (layer1[0] <= fxp_float_to_fxp(-0.83616900444)));

  float w1[4] = {0.195045f, 0.474309f, -0.148397f, -0.079880f};
  float b1 = -0.016677f;
  fxp_t r1 = perform(w1, 4, i, 4, b1);
  layer1[1] = r1;
  __ESBMC_assume((layer1[1] >= fxp_float_to_fxp(0.892706990242)) &&
                 (layer1[1] <= fxp_float_to_fxp(0.899767994881)));

  float w2[4] = {0.461122f, -0.476375f, 0.248677f, -0.553619f};
  float b2 = -0.017581f;
  fxp_t r2 = perform(w2, 4, i, 4, b2);
  layer1[2] = r2;
  __ESBMC_assume((layer1[2] >= fxp_float_to_fxp(0.145363003016)) &&
                 (layer1[2] <= fxp_float_to_fxp(0.212702006102)));

  float w3[4] = {-0.324553f, -0.542259f, 1.332844f, 0.068103f};
  float b3 = -0.118043f;
  fxp_t r3 = perform(w3, 4, i, 4, b3);
  layer1[3] = r3;
  __ESBMC_assume((layer1[3] >= fxp_float_to_fxp(-0.443677008152)) &&
                 (layer1[3] <= fxp_float_to_fxp(-0.367992997169)));

  float w4[4] = {0.405520f, 0.221390f, -0.391765f, 0.337188f};
  float b4 = -0.018501f;
  fxp_t r4 = perform(w4, 4, i, 4, b4);
  layer1[4] = r4;
  __ESBMC_assume((layer1[4] >= fxp_float_to_fxp(0.785434007645)) &&
                 (layer1[4] <= fxp_float_to_fxp(0.805330991745)));

  float w5[4] = {0.651486f, -0.367779f, -0.224814f, -0.037613f};
  float b5 = 0.077769f;
  fxp_t r5 = perform(w5, 4, i, 4, b5);
  layer1[5] = r5;
  __ESBMC_assume((layer1[5] >= fxp_float_to_fxp(0.419923990965)) &&
                 (layer1[5] <= fxp_float_to_fxp(0.461252003908)));

  float w6[4] = {0.981716f, 0.727559f, -1.363340f, -1.402215f};
  float b6 = 0.674765f;
  fxp_t r6 = perform(w6, 4, i, 4, b6);
  layer1[6] = r6;
  __ESBMC_assume((layer1[6] >= fxp_float_to_fxp(0.991369009018)) &&
                 (layer1[6] <= fxp_float_to_fxp(0.993957996368)));
  ...
  layer2[0] = r20;
  __ESBMC_assume((layer2[0] >= fxp_float_to_fxp(0.857891023159)) &&
                 (layer2[0] <= fxp_float_to_fxp(0.894620001316)));
  ...
  layer2[1] = r21;
  __ESBMC_assume((layer2[1] >= fxp_float_to_fxp(0.513521015644)) &&
                 (layer2[1] <= fxp_float_to_fxp(0.603668987751)));
  ...
  layer2[2] = r22;
  __ESBMC_assume((layer2[2] >= fxp_float_to_fxp(-0.418193012476)) &&
                 (layer2[2] <= fxp_float_to_fxp(-0.368425011635)));
