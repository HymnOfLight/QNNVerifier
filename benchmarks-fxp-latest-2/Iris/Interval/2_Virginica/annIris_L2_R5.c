...
int main() {
  ...
  int x0 = nondet_int();
  __ESBMC_assume((x0 >= 589) && (x0 <= 651));
  int x1 = nondet_int();
  __ESBMC_assume((x1 >= 323) && (x1 <= 357));
  int x2 = nondet_int();
  __ESBMC_assume((x2 >= 513) && (x2 <= 567));
  int x3 = nondet_int();
  __ESBMC_assume((x3 >= 219) && (x3 <= 241));
  ...
  layer1[0] = r0;
  __ESBMC_assume((layer1[0] >= fxp_float_to_fxp(-0.974180996418)) &&
                 (layer1[0] <= fxp_float_to_fxp(-0.969325006008)));

  float w1[4] = {0.195045f, 0.474309f, -0.148397f, -0.079880f};
  float b1 = -0.016677f;
  fxp_t r1 = perform(w1, 4, i, 4, b1);
  layer1[1] = r1;
  __ESBMC_assume((layer1[1] >= fxp_float_to_fxp(0.784551024437)) &&
                 (layer1[1] <= fxp_float_to_fxp(0.816690981388)));

  float w2[4] = {0.461122f, -0.476375f, 0.248677f, -0.553619f};
  float b2 = -0.017581f;
  fxp_t r2 = perform(w2, 4, i, 4, b2);
  layer1[2] = r2;
  __ESBMC_assume((layer1[2] >= fxp_float_to_fxp(-0.660838007927)) &&
                 (layer1[2] <= fxp_float_to_fxp(-0.551337003708)));

  float w3[4] = {-0.324553f, -0.542259f, 1.332844f, 0.068103f};
  float b3 = -0.118043f;
  fxp_t r3 = perform(w3, 4, i, 4, b3);
  layer1[3] = r3;
  __ESBMC_assume((layer1[3] >= fxp_float_to_fxp(0.796950995922)) &&
                 (layer1[3] <= fxp_float_to_fxp(0.866035997868)));

  float w4[4] = {0.405520f, 0.221390f, -0.391765f, 0.337188f};
  float b4 = -0.018501f;
  fxp_t r4 = perform(w4, 4, i, 4, b4);
  layer1[4] = r4;
  __ESBMC_assume((layer1[4] >= fxp_float_to_fxp(0.855710029602)) &&
                 (layer1[4] <= fxp_float_to_fxp(0.888045012951)));

  float w5[4] = {0.651486f, -0.367779f, -0.224814f, -0.037613f};
  float b5 = 0.077769f;
  fxp_t r5 = perform(w5, 4, i, 4, b5);
  layer1[5] = r5;
  __ESBMC_assume((layer1[5] >= fxp_float_to_fxp(0.0742629989982)) &&
                 (layer1[5] <= fxp_float_to_fxp(0.199777007103)));

  float w6[4] = {0.981716f, 0.727559f, -1.363340f, -1.402215f};
  float b6 = 0.674765f;
  fxp_t r6 = perform(w6, 4, i, 4, b6);
  layer1[6] = r6;
  __ESBMC_assume((layer1[6] >= fxp_float_to_fxp(-0.957018017769)) &&
                 (layer1[6] <= fxp_float_to_fxp(-0.897982001305)));
  ...
  layer2[0] = r20;
  __ESBMC_assume((layer2[0] >= fxp_float_to_fxp(0.389322012663)) &&
                 (layer2[0] <= fxp_float_to_fxp(0.577230989933)));
  ...
  layer2[1] = r21;
  __ESBMC_assume((layer2[1] >= fxp_float_to_fxp(0.156991004944)) &&
                 (layer2[1] <= fxp_float_to_fxp(0.364962995052)));
  ...
  layer2[2] = r22;
  __ESBMC_assume((layer2[2] >= fxp_float_to_fxp(0.545884013176)) &&
                 (layer2[2] <= fxp_float_to_fxp(0.639720022678)));
