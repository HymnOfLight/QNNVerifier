...
def read_vnnlib_simple(vnnlib_filename, num_inputs, num_outputs, regression=False):
    ...
    #    print(f"-----\n{i+1}. {box}\nspec:{spec_list}")
    print([spec[0] for spec in final_rv])

    return final_rv
#def get_variables_type(vnnlib, num_inputs, num_outputs):


#def convert_into_esbmc(vnnlib, num_inputs, num_outputs, regression=False):
...
if __name__ == "__main__":
    filename = input("please input file name and path")
    read_statements(filename)
    read_vnnlib_simple(filename,5,5)
    main()